...
%class lexer_FigurasCarpetProyecto
...
<YYINITIAL>{
    ...
    "*"|"+"|"-"|"/"|"("|")"|","             {anadirLexemaAnterior(yytext());
                                            return new Symbol( (yytext().equals("*")?MULT:(yytext().equals("/")?DIV:(yytext().equals("-")?RES:(yytext().equals("+")?SUM:(yytext().equals("(")?APERT:(yytext().equals(")")?CIER:COMA)))))), yyline +1, yycolumn +1);}      
